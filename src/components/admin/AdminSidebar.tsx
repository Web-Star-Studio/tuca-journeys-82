...
const AdminSidebar = ({ collapsed, setCollapsed }: AdminSidebarProps) => {
  ...
  const sidebarItems: SidebarItem[] = [
    ...
    { title: "Passeios", icon: Calendar, path: "/admin/tours" },
    { title: "Hospedagens", icon: Home, path: "/admin/accommodations" },
    { title: "Pacotes", icon: Package, path: "/admin/packages" },
    { title: "Produtos", icon: Store, path: "/admin/products", dividerAfter: true },
    { title: "Reservas", icon: ShoppingCart, path: "/admin/bookings" },
  ...
  return (
    <div
      className={`fixed h-screen bg-white shadow-md transition-all duration-300 z-40 ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      <div className="flex h-16 items-center justify-between px-4 border-b">
        <Link to="/admin" className="flex items-center">
          {!collapsed && (
            <span className="text-xl font-bold text-tuca-deep-blue">
              Tucano Admin
            </span>
          )}
        ...
        <Button
          ...
          className="text-gray-500 hover:text-tuca-ocean-blue"
        >
            ...
            <React.Fragment key={item.path}>
              <li>
                <Link
                  to={item.path}
                  className={cn(
                    "flex items-center px-4 py-3 text-gray-700 hover:bg-tuca-light-blue/40 hover:text-tuca-ocean-blue",
                    location.pathname === item.path &&
                      "bg-tuca-light-blue/60 text-tuca-ocean-blue font-medium"
                  )}
                >
