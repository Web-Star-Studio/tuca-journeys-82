...
const AdminSidebar = ({
  ...
  return (
    <div className={`fixed h-screen bg-white shadow-md transition-all duration-300 z-40 ${collapsed ? "w-20" : "w-64"}`}>
      <div className="flex h-16 items-center justify-between px-4 border-b">
        <Link to="/admin" className="flex items-center">
          {!collapsed && <span className="text-xl font-bold text-black">Administrativo</span>}
          {collapsed && <ArrowRight size={20} className="text-tuca-ocean-blue mx-auto" />}
        </Link>
            ...
            <React.Fragment key={item.path}>
              <li>
                <Link
                  to={item.path}
                  className={cn(
                    "flex items-center py-3 text-gray-700 hover:bg-tuca-light-blue/40 hover:text-tuca-ocean-blue",
                    collapsed ? "justify-center px-2" : "px-4",
                    location.pathname === item.path && "bg-tuca-light-blue/60 text-tuca-ocean-blue font-medium"
                  )}
                >
      ...
      <div className="absolute bottom-5 w-full px-4">
        <Button 
          variant="ghost" 
          className={cn(
            "flex w-full items-center py-3 text-gray-700 hover:bg-red-50 hover:text-red-600",
            collapsed ? "justify-center px-2" : "px-4"
          )} 
