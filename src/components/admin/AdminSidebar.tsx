...
const AdminSidebar = ({
  ...
  return <div className={`fixed h-screen bg-white shadow-md transition-all duration-300 z-40 ${collapsed ? "w-20" : "w-64"}`}>
      ...
      <div className="py-4">
        <ul className="space-y-1">
          {sidebarItems.map(item => <React.Fragment key={item.path}>
              <li>
                <Link to={item.path} className={cn("flex items-center px-4 py-3 text-gray-700 hover:bg-tuca-light-blue/40 hover:text-tuca-ocean-blue", location.pathname === item.path && "bg-tuca-light-blue/60 text-tuca-ocean-blue font-medium")}>
      ...
      <div className="absolute bottom-5 w-full px-4">
        <Button variant="ghost" className={cn("flex w-full items-center justify-start px-4 py-3 text-gray-700 hover:bg-red-50 hover:text-red-600", collapsed && "justify-center")} onClick={handleSignOut}>
