...
export function generateBookingsReportData(): BookingsReportData {
  try {
    ...
    console.error("Error generating bookings report data:", error);
    // Return simple fallback data
    const bookingsData: BookingData[] = [
      { month: "Jan", passeios: 100, hospedagens: 80, pacotes: 50, total: 230 },
      { month: "Fev", passeios: 110, hospedagens: 85, pacotes: 55, total: 250 },
    ];
    const statusData: StatusData[] = [
      { name: "Confirmadas", value: 400, color: "#10b981" },
      { name: "Pendentes", value: 80, color: "#f59e0b" },
      { name: "Canceladas", value: 40, color: "#ef4444" },
    ];
    return {
      bookingsData,
      statusData,
