...
import { useTourAvailability } from '@/hooks/use-tours';

interface TourAvailabilityCalendarProps {
  tourId?: number;
}

const TourAvailabilityCalendar: React.FC<TourAvailabilityCalendarProps> = ({ tourId }) => {
      ...
      return (
        <div className="w-full h-full flex flex-col items-center justify-center">
          <div className={`text-xs font-medium ${dayAvailability.status === 'available' ? 'text-green-600' : 'text-red-600'}`}>
            {dayAvailability.status === 'available' ? `${dayAvailability.available_spots} vagas` : 'Indisponível'}
          </div>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <span>Disponibilidade do Passeio</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {isLoading ? (
