              ...
              {wishlist.map((item) => (
                <Card key={`${item.type}-${item.id}`} className="overflow-hidden">
                  ...
                  <CardContent className="pt-4">
                    <h3 className="text-xl font-medium">{item.title || item.name}</h3>
                    ...
                    <p className="font-medium text-tuca-ocean-blue">
                      ...
                      item.price_per_night ? `R$ ${item.price_per_night.toFixed(2)} / noite` : ''}
                    </p>
                  </CardContent>
                  <CardFooter className="flex justify-between pt-2">
                    <Button variant="outline" size="sm" onClick={() => removeFromWishlist(item)}>
